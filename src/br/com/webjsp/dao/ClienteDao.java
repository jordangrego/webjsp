...
public class ClienteDao extends AbstractDao {
	...
	public List<Cliente> pesquisar(Cliente clienteCriterio) throws SQLException {
		...
		/*
		if (clienteCriterio.getIdLote() > 0) {
			sql.append(" id_lote = " + clienteCriterio.getIdLote() + " AND ");
		}

		if (clienteCriterio.getIdCliente() > 0) {
			sql.append(" id_cliente = " + clienteCriterio.getIdCliente() + " AND ");
		}

		if (clienteCriterio.getIdUsuario() > 0) {
			sql.append(" id_usuario = " + clienteCriterio.getIdUsuario() + " AND ");
		}

		if (clienteCriterio.getNumeroLote() > 0) {
			sql.append(" numero_lote = " + clienteCriterio.getNumeroLote() + " AND ");
		}

		if (clienteCriterio.getQtdCaixa() > 0) {
			sql.append(" qtd_caixa = " + clienteCriterio.getQtdCaixa() + " AND ");
		}

		if (clienteCriterio.getObservacao() != null && !clienteCriterio.getObservacao().equals("")) {
			sql.append(" observacao like '%" + clienteCriterio.getObservacao() + "%' AND ");
		}
